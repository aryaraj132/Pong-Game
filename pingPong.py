...
def message(msg,color):
    ...
    screen.blit(info, [width/3, height+60])
...
while running:
    ...
    if keys[pg.K_s] and y2<height-70:
        y2+=20
    if keys[pg.K_w] and y2>0:
        y2-=20
    if keys[pg.K_UP] and y1>0:
        y1-=20
    if keys[pg.K_DOWN] and y1<height-70:
        y1+=20
    x_change = boxSize
    y_change = boxSize
    if x_cod >= width:
    ...
    message("Press q to quit game", (255,0,0))
    pg.display.update()
    clock.tick(15)
